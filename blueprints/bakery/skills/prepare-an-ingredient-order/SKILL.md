...
Turns your shelf and walk-in counts into a per-supplier order sheet with a ready-to-send message for each supplier — ordering costs money, so placing every order stays your move.

## When to run this

Run this when the flour bin is looking low, when you do your regular count, or a day or two before a supplier's usual order day.

A steady rhythm helps. Many bakers count on the same quiet afternoon each week. If you tell Claude your rhythm, it can fold a count reminder into your morning brief.

## What Claude reads first

- business/suppliers.md — who supplies what, the reorder point for each item, usual order amounts, pack sizes, and how each supplier likes to receive orders.
- business/profile.md — your bakery's name for the order messages.
- reports/ — the latest bake list or weekly plan, if one exists, so a big week ahead can be raised as a question, never an assumption.

From you: today's counts. Read them out or paste them in any shape you like — "flour: [count] bags, butter: two blocks left, boxes: almost out" is plenty. Claude asks about anything unclear rather than guess.
