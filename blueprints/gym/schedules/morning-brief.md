...
In Claude Desktop, create a scheduled task and pick a time before you open — half an hour before the first class works well. Paste the prompt below as the task's instructions, exactly as written.

You don't need the schedule, though. Any morning, open this folder and say the prompt yourself. Same brief either way.

One thing the brief needs: today's class list and bookings. Paste it into the chat, or save the day's list from your booking tool into the inbox/ folder the night before. If Claude can't find today's list, it asks you for it instead of guessing.
...
Time for the morning brief. Read the business/ folder first — it is the source of truth for the gym.

Then:

1. Read yesterday's end-of-day report in reports/, if there is one.
2. Find today's class list and bookings. Check inbox/ for a fresh copy, or use what I have pasted here. If you can't find today's list, stop and ask me for it. Do not guess.
3. Write a brief of 6 to 8 lines covering: which classes run today and who coaches each, which classes have space, anything to set up or prep, follow-ups still waiting on me, and one small suggestion for the day.
